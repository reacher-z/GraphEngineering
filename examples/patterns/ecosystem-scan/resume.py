#!/usr/bin/env python3
"""Pattern 10 — injected failure and resume.  Python lane.

Two failures are injected for real here.  Neither is described; both happen.

A. **A fetch fails and is retried.**  The ``registry`` fetch's mock adapter is
...
B. **The process is lost mid-run and the run is resumed.**  A journal subclass
   ...
   ``fetch-advisories`` has a committed ``NodeSucceeded``.  ``start_graph_run``
   raises ``DURABILITY_STORE_FAILED``.  The run is then resumed against the
   same journal with a handler for ``fetch-advisories`` that fails if it is
   ever invoked.  It is not invoked: the committed fetch is reused, not
   re-fetched.
...
There is no overlap lease here: if this script were started twice against one
shared journal, nothing in this repository would fence the second run.  See
the bundle manifest's ``limitations.noOverlapLease``.

Run: uv run --project python python examples/patterns/ecosystem-scan/resume.py
"""
...
IMPLEMENTATION = "ecosystem-scan-mock@1"
RATE_LIMITED_RUN_ID = "ecosystem-scan-rate-limited-001"
RESUME_RUN_ID = "ecosystem-scan-resume-001"
...
async def main() -> None:
    ...
    # ------------------------------------------------------------------
    registry = corpus["sources"]["registry"]
    injected = Handlers(
        corpus,
        descriptor_document,
        scripts={
            ...
            "registry": (
                ...
                MockOutcome(
                    text=f"source registry reported {len(registry['items'])} items",
                    structured_output={
                        **registry,
                        "sourceKey": "registry",
    ...
    assert retry_run.outputs == expected_run["output"], "a retried fetch must not change the digest"
    assert injected.dispatches == 4, "three fetches, one of them dispatched twice"
    assert [attempt for key, attempt in injected.attempts if key == "registry"] == [1, 2]
    ...
    registry_node = retry_run.nodes["fetch-registry"]
    assert registry_node.status.value == "succeeded"
    assert registry_node.attempts == 2, "the injected rate limit must have cost a real attempt"
    ...
    journal = CommitThenLoseProcess(
        lambda batch: any(
            event_type == "NodeSucceeded" and node_id == "fetch-advisories"
    ...
    assert resumed.outputs == expected_run["output"], "the resumed digest must equal the nominal one"
        ...
        json.dumps(
            {
                ...
                "injectedSourceFailure": {
                    ...
                    "registryAttempts": [
                        attempt for key, attempt in injected.attempts if key == "registry"
                    ],
                ...
                "crashAndResume": {
                    ...
                    "committedFetchesBeforeResume": committed_fetches,
                    "resumeTail": [
