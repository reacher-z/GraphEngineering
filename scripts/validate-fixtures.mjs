...
import { validateRuntimeCapabilityFixture } from
  "../spec/conformance/runtime-capability.validate.mjs";
import { validateIntegratedBarrierFixture } from
  "../spec/conformance/integrated-barrier.validate.mjs";
import { validateRedactionFixture } from
  "../spec/conformance/redaction.validate.mjs";
import { budgetCampaign } from
  "../spec/conformance/budget.validate.mjs";
import { subgraphEdgeCampaign } from
  "../spec/conformance/subgraph-edge.validate.mjs";
import { validateApprovalFixture } from
  "../spec/conformance/approval.validate.mjs";
import { validateDurableExtensionFixture } from
  "../spec/conformance/durable-extension.validate.mjs";
import { validateIsolationFixture } from
  "../spec/conformance/isolation.validate.mjs";
import { validateVerificationFixture } from
  "../spec/conformance/verification.validate.mjs";
import { validateAdapterFixture } from
  "../spec/conformance/adapter.validate.mjs";
...
const cursorPublicationCampaign = validateCanonicalCursorPublicationFixture();
const runtimeCapabilityCampaign = await validateRuntimeCapabilityFixture();
const integratedBarrierCampaign = await validateIntegratedBarrierFixture();
const redactionCampaign = await validateRedactionFixture();
// Each of the five contract oracles below is invoked on its default validation
// path only: no guard-neutralization sweep runs here. Those sweeps
// (approval `sweepGuardNeutralization`, verification `measureGuardSurvivability`,
// durable-extension `GE_DX_MEASURE=1`) re-run the whole corpus once per guard and
// are reachable only from each validator's own CLI entry point, so this gate stays
// a validation gate rather than a multi-minute measurement sweep.
const approvalCampaign = await validateApprovalFixture();
const durableExtensionCampaign = validateDurableExtensionFixture();
const isolationCampaign = await validateIsolationFixture();
const verificationCampaign = await validateVerificationFixture();
const adapterCampaign = await validateAdapterFixture();
...
process.stdout.write(
  `Validated ${fixtureNames.length} JSON fixtures (${caseNames.length} case manifests), ${yamlNames.length} referenced YAML fixtures, ${Object.keys(expected.canonicalization).length} graph hash, ${Object.keys(expected.checkpoints ?? {}).length} checkpoint hash, ${durableJson.validCases.length} Durable JSON vectors, ${compiledIdentities.length} compiled identities, ${runtimeCapabilityCampaign.cases.length} runtime capability truth cases, the integrated-barrier contract corpus (${integratedBarrierCampaign.policyCases} policy, ${integratedBarrierCampaign.compilerCases} compiler, ${integratedBarrierCampaign.evaluationCases} evaluation, ${integratedBarrierCampaign.identityCases} identity, ${integratedBarrierCampaign.replayCases} replay cases; ${integratedBarrierCampaign.reasonCodesExercised}/11 reason codes, claims ${JSON.stringify(integratedBarrierCampaign.claims)}), ${graphPatchCases.validCases.length + graphPatchCases.invalidCases.length} graph patch schema cases plus ${graphPatchCases.semanticCases.length} closed semantic vectors, ${hostileShapeCases.length} hostile GraphPatch shape attacks, ${hostileSemanticCases.length} schema-valid hostile GraphPatch semantic/behavior cases, ${hostileRestoreCases.length} hostile GraphPatch replay/restore cases, 1 CycleStore provider descriptor schema with ${cycleStoreProviderCases.cases.length} closed cases plus ${sqliteCycleStoreCases.cases.length} SQLite durability cases and ${operationLedgerReplayCampaign.caseCount} contract-frozen SQLite operation-ledger replay cases (${operationLedgerReplayCampaign.behaviorCaseCount}/${operationLedgerReplayCampaign.attackCaseCount}), the ${cursorPublicationCampaign.ruleCount}-rule ${cursorPublicationCampaign.taskId} contract with ${cursorPublicationCampaign.hostileObligationCount} hostile obligations and ${cursorPublicationCampaign.faultBoundaryCount} fault boundaries, and 7 D7 controller/revision/event/checkpoint/lineage schemas with ${durableEvents.length} chained event goldens, ${cycleLineageCases.cases.length} lineage replay/corruption cases, ${cycleFaultMatrix.length} retained durable fault obligations, ${cycleInterruptionMatrix.length} activity interruption obligations, ${cycleOperationInterruptionMatrix.length} public-operation interruption obligations, ${cyclePatchVisibilityMatrix.length} patch-visibility fault obligations, ${cyclePatchCheckpointMatrix.length} patch-checkpoint fault obligations, ${cycleDurableCases.leaseTransitionCases.length} valid and ${cycleDurableCases.invalidLeaseTransitionCases.length} hostile lease transitions, ${cycleDurableCases.validInterruptedHistoryCases?.length ?? 0} interrupted terminal/checkpoint folds, ${cycleDurableCases.inDoubtProjectionCases.length} in-doubt singleton cases, ${resolutionProtocol.cases.length} terminal in-doubt resolution cases, ${cycleDurableCases.untilDryFoldCases.length} global-seen convergence fold, ${cycleDurableCases.hardStopFoldCases.length} hard-stop folds, ${cycleDurableCases.invalidEventHistoryCases.length} hostile histories, ${cycleDurableCases.invalidCheckpointSemanticCases.length} hostile checkpoint folds, plus ${cycleDurableCases.validStandaloneEventSchemaCases.length} standalone phase-event shapes; all ${expectedD7Tests.length} D7-CYCLE-SPEC-024 expected-test groups are mapped against meta-valid schemas. D9-REDACTION-039 contract corpus (${redactionCampaign.schemas} schemas, ${redactionCampaign.sourceClasses}x${redactionCampaign.sinkClasses}=${redactionCampaign.cartesianPairs} classified pairs, ${redactionCampaign.pointerCases} executed RFC 6901 pointer cases, ${redactionCampaign.wireCases} wire cases, ${redactionCampaign.flowCases} executed flow cases, ${redactionCampaign.policyEnablementCases} executed policy-enablement cases, ${redactionCampaign.executedSemanticCases}/${redactionCampaign.semanticPairs * 2} executed semantic cases; implementationClaim ${redactionCampaign.implementationClaim}), D10-BUDGET-SPEC-035 contract corpus (${budgetCampaign.schemas} schemas, ${budgetCampaign.vectors} vectors, ${budgetCampaign.routeCases} route cases, ${budgetCampaign.ledgerEvents} ledger events, ${budgetCampaign.semanticNegatives} semantic negatives; implementationClaim ${budgetCampaign.implementationClaim}), and D4-TRACE-SUBGRAPH-022 contract corpus (${subgraphEdgeCampaign.schemas} schemas, ${subgraphEdgeCampaign.boundGraphs} bound graphs, ${subgraphEdgeCampaign.events} chained events, ${subgraphEdgeCampaign.schemaNegatives} schema negatives, ${subgraphEdgeCampaign.runtimeCases} runtime-semantic cases; implementationClaim ${subgraphEdgeCampaign.implementationClaim}). D9-APPROVAL-077 contract corpus (${approvalCampaign.authorityCases} authority cases, ${approvalCampaign.scopeCases} scope cases, ${approvalCampaign.identityPerturbations} identity perturbations, ${approvalCampaign.presentationCases} presentation cases, ${approvalCampaign.waitingCases} waiting cases, ${approvalCampaign.dischargeCases} discharge cases, ${approvalCampaign.envelopeCases} envelope cases, ${approvalCampaign.errorCodesExercised} error codes and ${approvalCampaign.guardsExercised}/${approvalCampaign.guards} guards exercised, ${approvalCampaign.recomputedLiterals} recomputed literals; claims ${JSON.stringify(approvalCampaign.claims)}), D9-DURABLE-EXT-SPEC-031 contract corpus (${durableExtensionCampaign.schemas} schemas, ${durableExtensionCampaign.runs} hash-chained runs, ${durableExtensionCampaign.frames} authority frames, ${durableExtensionCampaign.checkpoints} checkpoints, ${durableExtensionCampaign.replayPlans} replay plans, ${durableExtensionCampaign.rules} rejection rules, ${durableExtensionCampaign.semanticNegatives} semantic negatives, ${durableExtensionCampaign.schemaNegatives} schema negatives; implementationClaim ${durableExtensionCampaign.implementationClaim}), D12-ISOLATION-SPEC-044 contract corpus (${isolationCampaign.schemas} schemas, ${isolationCampaign.cases} cases, ${isolationCampaign.codes} codes, ${isolationCampaign.reasons} reasons, ${isolationCampaign.escapes} bound escape classes; implementationClaim ${isolationCampaign.implementationClaim}), D11-VERIFY-SPEC-040 contract corpus (${verificationCampaign.rubrics} rubrics, ${verificationCampaign.verdicts} verdicts, ${verificationCampaign.panelCases} panel cases, ${verificationCampaign.judgeCases} judge cases, ${verificationCampaign.citationCases} citation cases, ${verificationCampaign.reflectionRecords} reflection records, ${verificationCampaign.modeBCases} mode-B cases, ${verificationCampaign.replayCases} replay cases, ${verificationCampaign.schemaNegativeCases} schema negatives, ${verificationCampaign.guardCases} guard vectors over ${verificationCampaign.guardsWithVectors}/${verificationCampaign.guards} guards, ${verificationCampaign.recomputedLiterals} recomputed literals; implementationClaim ${verificationCampaign.implementationClaim}), and D13-ADAPTER-SPEC-048 contract corpus (${adapterCampaign.schemas} schemas, ${adapterCampaign.descriptors} descriptors, ${adapterCampaign.descriptorCases} descriptor cases, ${adapterCampaign.preflightCases} preflight cases, ${adapterCampaign.toolCases} tool cases, ${adapterCampaign.streamCases} stream cases, ${adapterCampaign.usageCases} usage cases, ${adapterCampaign.errorCases} error cases, ${adapterCampaign.retryCases} retry cases, ${adapterCampaign.circuitCases} circuit cases, ${adapterCampaign.schemaNegativeCases} schema negatives, ${adapterCampaign.rulesExercised}/${adapterCampaign.rules} rules exercised; implementationClaim ${adapterCampaign.implementationClaim}).\n`,
);
