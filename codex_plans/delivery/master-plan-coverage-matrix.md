...
| 6 — terminal semantics and quorum | Partial; barrier contract frozen, nothing implemented | Structured scheduler and pipeline failures; retry/timeout/cancellation; attempt budgets; upstream isolation. `spec/integrated-barrier-semantics.md` plus four schemas freeze barriers, quorum, abstention, deadlines, the closed `onUnsatisfied` set, late arrival and zero-rejudge decision replay at `implementationClaim: false` | Every planned node/edge terminal state, quorum/abstention at runtime, human/unknown escalation, failure-injection matrix. `scheduler.ts` still classifies `barrier` with `transform`, so a declared barrier policy is currently executed as a plain transform and an unsatisfied barrier is silently passed; the pre-dispatch capability rejection is the first required deliverable | `D6-ROUTER-BARRIER-023`, `D11-VERIFY-SPEC-040` |
...
| Barriers | Deterministic all/minimum/percentage settled evaluator; frozen integrated contract candidate at `implementationClaim: false` | Every runtime behavior: durable wait, deadline, quorum, abstention, missing statistics, the three non-pass resolutions, late arrival and cancellation propagation. The scheduler still executes `barrier` as a transform | Partial; contract only |
